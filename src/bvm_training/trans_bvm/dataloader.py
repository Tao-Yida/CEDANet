...
def _setup_reproducibility(freeze, random_seed, verbose=True):
    ...
    if freeze:
        ...
        if verbose:
            print(f"[FREEZE MODE] All randomness is frozen, seed={random_seed}, shuffle is disabled")
        return False  # Return shuffle=False
    return True  # Return shuffle=True
...
def get_dataset_name_from_path(dataset_path):
    ...
    # If no parts remain after filtering, use the last two parts of the original path
...
def get_train_val_loaders(
    ...
    Args:
        ...
        freeze: Whether to freeze all randomness to ensure full reproducibility, default False
        shuffle: Whether to shuffle training data (overridden to False when freeze=True)
    ...
    val_dataset = SalObjDataset.from_indices(
        images, gts, trans, val_indices, trainsize, aug=False, freeze=freeze
...
class SalObjDataset(data.Dataset):
    """Unified salient object dataset class"""

    def __init__(self, all_images, all_gts, all_trans, trainsize, indices=None, aug=False, freeze=False):
        """
        Args:
            all_images: All image path list
            all_gts: All GT path list
            all_trans: All transmission map path list
            trainsize: Training image size
    ...
    def _setup_transforms(self):
        ...
        # Color augmentation (applied only when needed)
        if self.aug and not self.freeze:
            ...
            if self.freeze and self.aug:
                print("[FREEZE MODE] Data augmentation is disabled to ensure full reproducibility")
    ...
    def __getitem__(self, index):
        ...
        # Apply color transform (only to image)
    ...
    def _filter_files(self):
        """Filter out files with mismatched sizes"""
        assert len(self.images) == len(self.gts) == len(self.trans), "Mismatched number of images, GTs, and transmission maps"
