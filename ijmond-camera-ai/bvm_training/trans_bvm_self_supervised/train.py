...
def argparser():
    ...
    parser.add_argument(
        "--inter", type=bool, default=False, help="Inter pixel (differenct image) match if True, else intra pixel (same image) match."
    )
...
if __name__ == "__main__":
    ...
    print(f"  - Generator: {opt.lr_gen}")
    print(f"  - Descriptor: {opt.lr_des}")
    print("\nOptimization Settings:")
    print(f"  - Batch Size: {opt.batchsize}")
    print(f"  - Training Size: {opt.trainsize}")
    print(f"  - Gradient Clip: {opt.clip}")
    print(f"  - Adam Beta: {opt.beta}")
    print("\nLearning Rate Schedule:")
    print(f"  - Decay Rate: {opt.decay_rate}")
    print(f"  - Decay Epoch: {opt.decay_epoch}")
    print("\nModel Architecture:")
    print(f"  - Generator Reduced Channel: {opt.gen_reduced_channel}")
    print(f"  - Descriptor Reduced Channel: {opt.des_reduced_channel}")
    print(f"  - Feature Channel: {opt.feat_channel}")
    print(f"  - Latent Dimension: {opt.latent_dim}")
    print("\nLoss Weights:")
    print(f"  - Smoothness Weight: {opt.sm_weight}")
    print(f"  - Regularization Weight: {opt.reg_weight}")
    print(f"  - Latent Loss Weight: {opt.lat_weight}")
    print(f"  - VAE Loss Weight: {opt.vae_loss_weight}")
    print("\nPaths:")
    print(f"  - Labeled Dataset Path: {opt.labeled_dataset_path}")
    print(f"  - Unlabeled Dataset Path: {opt.unlabeled_dataset_path}")
    print(f"  - Save Model Path: {opt.save_model_path}")
    print(f"  - Pretrained Weights: {opt.pretrained_weights}")
    print("\nEBM Settings:")
    print(f"  - Langevin Steps: {opt.langevin_step_num_des}")
    print(f"  - Langevin Step Size: {opt.langevin_step_size_des}")
    print(f"  - Energy Form: {opt.energy_form}")
    print("==========================================\n")
    ...
    for epoch in range(1, opt.epoch + 1):
        ...
        for i, pack in enumerate(train_loader, start=1):
            ...
            for rate in size_rates:
                ...
                # Variable wrapper deprecated; use tensors directly
                # images_lb, gts_lb, trans_lb are already tensors with requires_grad=False
                images_lb = images_lb.cuda()
                gts_lb = gts_lb.cuda()
                trans_lb = trans_lb.cuda()
                # Unpack pseudo-labeled data
                images_un, gts_un, trans_un = pseudo_labeled_pack
                # Remove Variable wrapper for pseudo-labeled data
                ...
                if rate != 1:
                    # F.upsample is deprecated; use interpolate
                ...
                sal_loss = 0.5 * (
                    structure_loss(pred_post_init[:num_labeled_data], gts_lb) + structure_loss(pred_post_ref[:num_labeled_data], gts_lb)
                )
                ...
                if rate == 1:
                    # loss_record expects Python float or tensor
