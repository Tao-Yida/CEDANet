...
def predict_frames(model, image_paths, testsize, device):
    """
    预测多个图像帧
    Args:
        model: 预训练模型
...
def main():
    ...
    # 清理输出目录中的现有文件
    clean_output_directories(opt.output_path)
    ...
    for video_file in video_files:
        ...
        # 为每个视频创建临时帧目录
        temp_frames_dir = os.path.join(opt.output_path, f"temp_{video_name}")
        ...
        # 预测所有帧
        predictions = predict_frames(generator, frame_paths, opt.testsize, device)
        ...
        # 保存结果到统一目录，并获取下一个起始索引
        saved_files, next_idx = save_results(predictions, selected_frames, video_name, opt.output_path, next_idx)
    ...
    if all_saved_files:
        generate_transmission_maps(opt.output_path, all_saved_files)

    print("\n所有视频处理完成，伪标签和透光率图已生成")
